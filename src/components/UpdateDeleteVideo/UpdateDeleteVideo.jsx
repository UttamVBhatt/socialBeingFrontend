...
export function UpdateDeleteVideo({ dispatch, selectedVideo }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (!caption || !selectedVideo) return;

      const data = await axios({
        method: "PATCH",
        url: `${BASE_URL}/api/v1/video/posts/${selectedVideo}/${user?._id}`,
        data: { caption },
      });
  ...
  async function handleDelete(e) {
    ...
    try {
      ...
      const data = await axios({
        method: "DELETE",
        url: `${BASE_URL}/api/v1/video/posts/${selectedVideo}`,
      });
    ...
    <div className="update-data-container">
      ...
      <div className="update-data-form-container">
        <h3 onClick={() => setIsDelete(!isDelete)}>
          Update Video / <span style={{ color: "red" }}>Delete Video</span>
        </h3>
        <span onClick={() => dispatch({ type: "setIsVideoUpdateDelete" })}>
          X
        </span>
